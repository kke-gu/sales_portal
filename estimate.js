...
function initQuoteForm() {
    ...
    // 견적서 저장 이벤트 (estimate_modify.html에서는 제외)
    if (form && !window.location.pathname.includes('estimate_modify.html')) {
...
function saveEstimate() {
    ...
    const now = new Date();
    const registeredDate = now.toISOString(); // 등록일자 (ISO 형식, 시간 포함)
    ...
    const estimateData = {
        ...
        status: 'received', // 현황: 접수
        createdBy: currentUser || '', // 작성자
...
function saveTempEstimate() {
    ...
    const now = new Date();
    const registeredDate = now.toISOString();
